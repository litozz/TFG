		...
		if(!isNaN(cost)){
        	if(cost>=0){
				...
				switch(error){
					...
					case -2: alert("Uno (o ambos) de los dos nodos no existe.");break;//El código de error -2 me dice que uno de los dos no existe
					default: 
						this.is_initial_selected=false;
			            this.resetEdgeEntry();
			            if(!this.created_events){
	...
	exportNetwork(){
		try{
			if(this.is_initial_selected){
				...
				if(!this.grafo_principal.initialNodeExists()){
			    	alert("Debe haber un (y solo un) nodo inicial");
	        ...
	        switch(strUser) {
				...
				case "iterative-descent":
					try{
						...
						if(isNaN(prof_bound)){
	    					alert("Por favor, introduzca un valor numérico entre "+document.getElementById("max_prof").min+" y "+document.getElementById("max_prof").max+" en el campo máxima profundidad.");
	    				}else if(prof_bound<1 || prof_bound>document.getElementById("max_prof").max){
	    					alert("Por favor, introduzca un numero máximo de iteraciones entre "+document.getElementById("max_prof").min+" y "+document.getElementById("max_prof").max+".");
	    				}else{
	...
	generateHeuristic(){
		if(this.is_initial_selected){
			...
			if(!this.grafo_principal.initialNodeExists()){
            	...
            	alert("Debe introducir valores en los campos de rango si desea generar una heuristica que provoque cambios en cerrados.");
		    }else{
